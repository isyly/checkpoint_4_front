...
export default function InfiniteScroll() {
  ...
  const infiniteFetchData = () => {
    fetch(
      `https://api.unsplash.com/search/photos?page=${pageIndex}&per_page=30&query=${searchState}&client_id=2SHw_ss7MmQqrY1X_gPbIy0GNFcueYWlp4kKJnaI7VY`
    )
      .then((response) => {
  ...
  return (
    <div className="container">
      ...
      <div className="card-list">
        <div>
          {dataImg[0].map((img) => {
            return <img key={uuidv4()} src={img} alt="image_unsplash" />;
          })}
        </div>
        <div>
          {dataImg[1].map((img) => {
